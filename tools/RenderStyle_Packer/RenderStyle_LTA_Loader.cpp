...
//				else if (stricmp(szParam1,"RENDERSTYLE_COLOROP_HIGHLIGHT2")==0)				{ RenderPass.TextureStages[j].ColorOp = RENDERSTYLE_COLOROP_HIGHLIGHT2; }
			...
			if (pUVProject_Enable && pUVProject_Enable->GetElement(1) && (szParam1 = pUVProject_Enable->GetElement(1)->GetValue())) {
				if (stricmp(szParam1,"1")==0)												{ RenderPass.TextureStages[j].UVProject_Enable = true; }
				else if (stricmp(szParam1,"0")==0)											{ RenderPass.TextureStages[j].UVProject_Enable = false; }
				else { RenderPass.TextureStages[j].UVProject_Enable = false; OutputMsg("Warning: Unknown param %s",szParam1); } }
			else { RenderPass.TextureStages[j].UVProject_Enable = false; }
