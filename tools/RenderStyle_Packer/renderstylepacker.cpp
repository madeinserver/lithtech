...
void PackIt(string szInputFile[], const char* szOutputFile)
{
	...
	default:
		{
			char buf[256];
			std::sprintf(buf, "ltb file type : %i", g_PackType );
...
bool PreProcessShaderFile(const char* szFileName, string& OutputCode, uint32 iType)
{
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_IFRIGID<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_IFRIGID<")) {		// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_IFRIGID<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_IFSKIN<")) {		// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_IFSKIN<")) {		// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_IFSKIN<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		int32 iStartCh = pCh    - OutputCode.c_str();
		int32 iEndCh   = pEndCh - OutputCode.c_str();
		if (!bRigid) {
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_RIGIDTRANS4<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_RIGIDTRANS4<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_RIGIDTRANS4<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		int32 iStartCh = pCh    - OutputCode.c_str();
		int32 iEndCh   = pEndCh - OutputCode.c_str();
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_RIGIDTRANS3<")) {
		...
		if (bRigid) {
			std::sprintf(szBuff,"dp3 %s.x, %s, c%d\n",OutPos,InPos,iMatConst+0);			OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
			std::sprintf(szBuff,"dp3 %s.y, %s, c%d\n",OutPos,InPos,iMatConst+1);			OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
			std::sprintf(szBuff,"dp3 %s.z, %s, c%d\n",OutPos,InPos,iMatConst+2);			OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff); } }

	// LT_MACRO_SKINTRANS4 Macro...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_SKINTRANS4<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_SKINTRANS4<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_SKINTRANS4<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		int32 iStartCh = pCh    - OutputCode.c_str();
		int32 iEndCh   = pEndCh - OutputCode.c_str();
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_SKINTRANS3<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_SKINTRANS3<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_SKINTRANS3<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		int32 iStartCh = pCh    - OutputCode.c_str();
		int32 iEndCh   = pEndCh - OutputCode.c_str();
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_LIGHT_ATT<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_LIGHT_ATT<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_LIGHT_ATT<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		int32 iStartCh = pCh    - OutputCode.c_str();
		int32 iEndCh   = pEndCh - OutputCode.c_str();
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_SKINBLENDTRANS4<")) {
		...
		if (!bRigid) { 									// Expand the sucka...
			...
			case 3 :
				...
				std::sprintf(szBuff,"dp4 %s.z, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+3);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp4 %s.w, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+4);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"mad %s, %s, %s.zzzz, %s\n",OutPos,TmpReg,Weights,OutPos);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);

				std::sprintf(szBuff,"mul %s.x, %s.w, c[%d].w\n",TmpReg,Indicies,iStartConst);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"mov a0.x, %s\n",TmpReg);									OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp4 %s.x, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+1);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp4 %s.y, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+2);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp4 %s.z, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+3);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
	...
	while (pCh = (char*)strstr(OutputCode.c_str(),"//LT_MACRO_SKINBLENDTRANS3<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),";LT_MACRO_SKINBLENDTRANS3<")) {	// Erase the commented out ones...
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		OutputCode.erase((pCh - OutputCode.c_str()),(pEndCh - OutputCode.c_str())-(pCh - OutputCode.c_str())+1); }
	while (pCh = (char*)strstr(OutputCode.c_str(),"LT_MACRO_SKINBLENDTRANS3<")) {
		char* pEndCh = (char*)strstr(pCh,">"); if (!pEndCh) return false;
		int32 iStartCh = pCh    - OutputCode.c_str();
		int32 iEndCh   = pEndCh - OutputCode.c_str();
		...
		if (!bRigid) { 									// Expand the sucka...
			...
			case 1 :									// Two Bones...
				std::sprintf(szBuff,"mul %s.x, %s.x, c[%d].w\n",TmpReg,Indicies,iStartConst);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"mov a0.x, %s\n",TmpReg);									OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp3 %s.x, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+1);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp3 %s.y, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+2);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"dp3 %s.z, %s, c[a0.x + %d]\n",TmpReg,InPos,iStartConst+3);	OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
				std::sprintf(szBuff,"mul %s, %s, %s.xxxx\n",OutPos,TmpReg,Weights);				OutputCode.insert(iStartCh,szBuff);		iStartCh += strlen(szBuff);
