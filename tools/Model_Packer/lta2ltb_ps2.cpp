...
#define SCE_GS_SET_PRIM(prim, iip, tme, fge, abe, aa1, fst, ctxt, fix) \
	((uint32)(prim)      | ((uint32)(iip) << 3)  | ((uint32)(tme) << 4) | \
	((uint32)(fge) << 5) | ((uint32)(abe) << 6)  | ((uint32)(aa1) << 7) | \
	((uint32)(fst) << 8) | ((uint32)(ctxt) << 9) | ((uint32)(fix) << 10))
					...
					*m_pFile << c.dwordVal;
                    ...
                    if (vertexUses[vInd].empty()){
                        *m_pFile << zero32;
						...
						int i;
                        for (i=0;i<(int)vertexUses[vInd].size();++i){
