	...
	afx_msg void OnVertexShaderSaveAssembled()						{
		...
		if (BrowseBox.DoModal() == IDOK && BrowseBox.GetPathName()) { 
			...
			for (uint32 iType = 0; iType < VERTSHADER_TYPES_COUNT; ++iType) {
				...
				if (!PreProcessShaderFile(m_pDirect3DData->VertexShaderFilename.c_str(),szCode,iType) || D3DXAssembleShader(szCode.c_str(),szCode.size(),NULL,NULL, 0, &pVertShaderCode,&pErrors) != D3D_OK) { 
					string szTmp = g_StartingDirectory; szTmp += "\\"; szTmp += m_pDirect3DData->VertexShaderFilename.c_str();
					if (!PreProcessShaderFile(szTmp.c_str(),szCode,iType) || D3DXAssembleShader(szCode.c_str(),szCode.size(),NULL,NULL, 0, &pVertShaderCode,&pErrors) != D3D_OK) { 
