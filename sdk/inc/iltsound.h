...
typedef int32( *LTLENGTHYCB )	( uint32 uiState, uint32 uiUser );	//AILLENGTHYCB

typedef uint32			LHPROVIDER;			//HPROVIDER;
typedef uint32			LHPROENUM;			//HPROENUM;

typedef void*			LHSAMPLE;			//HSAMPLE;
typedef void* LH3DPOBJECT;		//H3DPOBJECT;
typedef void* LH3DSAMPLE;			//H3DSAMPLE;
typedef void* LHDIGDRIVER;		//HDIGDRIVER;
typedef void* LHSTREAM;			//HSTREAM;

typedef void* PHWAVEOUT;
typedef void* PTDIRECTSOUND;
typedef void* PTDIRECTSOUNDBUFFER;

typedef void* LDSOUND; //LPDIRECTSOUND8
typedef void* LDMUSICPERF; // LPDIRECTMUSICPERFORMANCE8
typedef void* LDMUSIC; // LPDIRECTMUSIC
...
typedef struct LWAVEFORMAT_lith : LWAVEFORMAT
{
...
{
	int32	m_siParams[ SBP_NUM_PARAMS ];
};
...
public:
	...
	virtual int32		Startup( void ) = 0;
	virtual void		Shutdown( void ) = 0;
	virtual uint32		MsCount( void ) = 0;
	virtual int32		SetPreference( uint32 uiNumber, int32 siValue ) = 0;
	virtual int32		GetPreference( uint32 uiNumber ) = 0;
	...
	virtual int32		WaveOutOpen( LHDIGDRIVER* phDriver, PHWAVEOUT* pphWaveOut, int32 siDeviceId, LWAVEFORMAT* pWaveFormat ) = 0;
	virtual void		WaveOutClose( LHDIGDRIVER hDriver ) = 0;
	virtual void		SetDigitalMasterVolume( LHDIGDRIVER hDig, int32 siMasterVolume ) = 0;
	virtual int32		GetDigitalMasterVolume( LHDIGDRIVER hDig ) = 0;
	virtual int32		DigitalHandleRelease( LHDIGDRIVER hDriver ) = 0;
	virtual int32		DigitalHandleReacquire( LHDIGDRIVER hDriver ) = 0;
#ifdef USE_EAX20_HARDWARE_FILTERS
	...
	virtual int32		Open3DProvider( LHPROVIDER hLib ) = 0;
	...
	virtual int32		Enumerate3DProviders( LHPROENUM* phNext, LHPROVIDER* phDest, char** psName) = 0;
	...
	virtual void		Set3DUserData( LH3DPOBJECT hObj, uint32 uiIndex, int32 siValue ) = 0;
	...
	virtual int32		Get3DUserData( LH3DPOBJECT hObj, uint32 uiIndex) = 0;
	...
	virtual int32		Init3DSampleFromAddress( LH3DSAMPLE hS, void* pStart, uint32 uiLen, LWAVEFORMATEX* pWaveFormat, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData  ) = 0;
	virtual int32		Init3DSampleFromFile( LH3DSAMPLE hS, void* pFile_image, int32 siBlock, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData ) = 0;
	virtual int32		Get3DSampleVolume( LH3DSAMPLE hS ) = 0;
	virtual void		Set3DSampleVolume( LH3DSAMPLE hS, int32 siVolume ) = 0;
	virtual uint32		Get3DSampleStatus( LH3DSAMPLE hS ) = 0;
	virtual void		Set3DSampleMsPosition( LHSAMPLE hS, int32 siMilliseconds ) = 0;
	virtual int32		Set3DSampleInfo( LH3DSAMPLE hS, LTSOUNDINFO* pInfo ) = 0;
	...
	virtual void		Set3DSampleLoopBlock( LH3DSAMPLE hS, int32 siLoop_start_offset, int32 siLoop_end_offset, bool bEnable ) = 0;
	...
	virtual void		SetSampleVolume( LHSAMPLE hS, int32 siVolume ) = 0;
	virtual void		SetSamplePan( LHSAMPLE hS, int32 siPan ) = 0;
	virtual int32		GetSampleVolume( LHSAMPLE hS ) = 0;
	virtual int32		GetSamplePan( LHSAMPLE hS ) = 0;
	virtual void		SetSampleUserData( LHSAMPLE hS, uint32 uiIndex, int32 siValue ) = 0;
	...
	virtual int32		InitSampleFromAddress( LHSAMPLE hS, void* pStart, uint32 uiLen, LWAVEFORMATEX* pWaveFormat, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData ) = 0;
	virtual int32		InitSampleFromFile( LHSAMPLE hS, void* pFile_image, int32 siBlock, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData ) = 0;
	virtual void		SetSampleLoopBlock( LHSAMPLE hS, int32 siLoop_start_offset, int32 siLoop_end_offset, bool bEnable ) = 0;
	virtual void		SetSampleLoop( LHSAMPLE hS, bool bLoop ) = 0;
	virtual void		SetSampleMsPosition( LHSAMPLE hS, int32 siMilliseconds ) = 0;
	virtual int32		GetSampleUserData( LHSAMPLE hS, uint32 uiIndex ) = 0;
	virtual uint32		GetSampleStatus( LHSAMPLE hS ) = 0;

	// old 2d sound stream functions
	virtual LHSTREAM	OpenStream( char* sFilename, uint32 nOffset, LHDIGDRIVER hDig, char* sStream, int32 siStream_mem ) = 0;
	virtual void		SetStreamLoop( LHSTREAM hStream, bool bLoop ) = 0;
	virtual void		SetStreamPlaybackRate( LHSTREAM hStream, int32 siRate ) = 0;
	virtual void		SetStreamMsPosition( LHSTREAM hS, int32 siMilliseconds ) = 0;
	virtual void		SetStreamUserData( LHSTREAM hS, uint32 uiIndex, int32 siValue) = 0;
	virtual int32		GetStreamUserData( LHSTREAM hS, uint32 uiIndex) = 0;
...
//	virtual LHSTREAM	OpenStream( streamBufferParams_t* pStreamBufferParams ) = 0;
	...
	virtual void		PauseStream( LHSTREAM hStream, int32 siOnOff ) = 0;
	virtual void		ResetStream( LHSTREAM hStream ) = 0;
	virtual void		SetStreamVolume( LHSTREAM hStream, int32 siVolume ) = 0;
	virtual void		SetStreamPan( LHSTREAM hStream, int32 siPan ) = 0;
	virtual int32		GetStreamVolume( LHSTREAM hStream ) = 0;
	virtual int32		GetStreamPan( LHSTREAM hStream ) = 0;
	virtual uint32		GetStreamStatus( LHSTREAM hStream ) = 0;
	virtual int32		GetStreamBufferParam( LHSTREAM hStream, uint32 uiParam ) = 0;
	virtual void		ClearStreamBuffer( LHSTREAM hStream, bool bClearStreamDataQueue = true) = 0;

	// wave file decompression functons
	virtual int32		DecompressADPCM( LTSOUNDINFO* pInfo, void** ppOutData, uint32* puiOutSize ) = 0;
	virtual int32		DecompressASI( void* pInData, uint32 uiInSize, char* sFilename_ext, void** ppWav, uint32* puiWavSize, LTLENGTHYCB fnCallback ) = 0;
