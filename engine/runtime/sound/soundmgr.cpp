...
LTRESULT CSoundMgr::Init(InitSoundInfo &soundInit)
{
    ...
    if ( g_pSoundSys->WaveOutOpen(&m_hDigDriver, 0, WAVE_MAPPER, ( LWAVEFORMAT* )&m_PrimaryBufferWaveFormat) != LS_OK )
    {
        Term();
        return LT_UNABLETOINITSOUND;
    }
...
// 
//----------------------------------------------------------------------------------------------
LDSOUND CSoundMgr::GetDirectSound()
{
    void* lpDirectSound;
...
void CSoundMgr::Release3DSample(LH3DSAMPLE h3DSample)
{
	if (!g_pSoundSys) return;
