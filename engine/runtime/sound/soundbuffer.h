...
public:
	...
	LWAVEFORMATEX*	GetWaveFormat( LTBOOL bDelegate = LTTRUE );



	float			RandomPitchMod( );

	int32				GetSampleType( LTBOOL bDelegate = LTTRUE )
	{ return ( bDelegate && m_pDecompressedSoundBuffer ) ? m_pDecompressedSoundBuffer->m_nSampleType : m_nSampleType; }

	int32				GetPlaybackRate( LTBOOL bDelegate = LTTRUE )
...
protected:
	...
	void				CalcSampleType( int32 &sampleType, LWAVEFORMATEX &waveFormat );
...
inline LWAVEFORMATEX* CSoundBuffer::GetWaveFormat( LTBOOL bDelegate )
{
