...
LTRESULT CSoundInstance::AcquireSample( )
{
	...
	nPlayBackRate = ( int32 )(( float )nPlayBackRate * m_fPitchShift + 0.5f );
	...
	if( m_dwPlaySoundFlags & PLAYSOUND_LOOP )
	{
		...
		if( m_pSoundBuffer->GetWaveHeader( LTFALSE ).m_CuePoint.m_bValid )
		{
			...
			nPos1 = ( int32 )(( fModPoint / ( float )m_dwDuration ) * 
				( float )m_pSoundBuffer->GetSoundDataLen( ));
			nPos1 = LTCLAMP( nPos1, ( int32 )0, ( int32 )m_pSoundBuffer->GetSoundDataLen( ));
			fModPoint = ( float )m_pSoundBuffer->GetLoopPoint( 1 ) / m_fPitchShift;
			nPos2 = ( int32 )(( fModPoint / ( float )m_dwDuration ) * 
				( float )m_pSoundBuffer->GetSoundDataLen( ));
			nPos2 = LTCLAMP( nPos2, nPos1, ( int32 )m_pSoundBuffer->GetSoundDataLen( ));
...
{
	const LWAVEFORMATEX *pWaveFormat;
	uint32 dwCurPos;
