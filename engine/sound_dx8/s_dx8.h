...
public:
	...
	LTVector	m_up;
	LTVector	m_face;
	int32		m_userData[ MAX_USER_DATA_INDEX + 1 ];
};
...
public:
	...
	bool Init( HRESULT& hResult, LPDIRECTSOUND pDS, uint32 uiNumSamples, 
		bool b3DBuffer, LWAVEFORMATEX* pWaveFormat = NULL, LTSOUNDFILTERDATA* pFilterData = NULL );
	void Term( );
	void Restore( );
	bool Fill( );
...
public:
	...
	virtual ~C3DSample( );
	void Reset( );
	bool Init( HRESULT& hResult, LPDIRECTSOUND pDS, uint32 uiNumSamples, LWAVEFORMATEX* pWaveFormat, LTSOUNDFILTERDATA* pFilterData );
	void Term( );
	virtual void SetPosition( LTVector& pos );
	virtual void SetVelocity( LTVector& vel );
	virtual void SetOrientation( LTVector& up, LTVector& face );
...
public:
	...
	virtual int32			Init3DSampleFromAddress( LH3DSAMPLE hS, void* pStart, uint32 uiLen, LWAVEFORMATEX* pWaveFormat, int32 nPitchShift, LTSOUNDFILTERDATA* pFilterData );
	...
	virtual LHSTREAM	OpenStream( char* sFilename, uint32 nFilePos, LHDIGDRIVER hDig, char* sStream, int32 siStream_mem );
	virtual void		SetStreamLoop( LHSTREAM hStream, bool bLoop );
	virtual void		SetStreamPlaybackRate( LHSTREAM hStream, int32 siRate );
	virtual void		SetStreamMsPosition( LHSTREAM hStream, int32 siMilliseconds );
...
//	Incorporation of DSMStrm* required functionality
public:
	CSample*			CreateBuffer( LWAVEFORMATEX* pWaveFormat, DWORD dwBufferSize, DWORD dwFlags );
...
public:
	int32					m_userPrefs[ MAX_USER_PREF_INDEX + 1 ];
