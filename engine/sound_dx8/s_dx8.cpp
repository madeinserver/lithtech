...
#undef USE_EAX20_HARDWARE_FILTERS // we do not have EAX inclusions
#if USE_EAX20_HARDWARE_FILTERS
#include "eax.h"
#endif
...
BOOL WaveFile::Open (LPSTR pszFilename, uint32 nFilePos)
{
	...
	if( m_mmr == MMSYSERR_NOERROR )
	{
		// Find format chunk ('fmt '), allocate and fill LWAVEFORMATEX structure
	...
	if( m_mmr == MMSYSERR_NOERROR )
	{
		// If format is not PCM, then there are extra bytes appended to LWAVEFORMATEX
	...
	if( m_mmr == MMSYSERR_NOERROR )
	{
		// Allocate memory for LWAVEFORMATEX structure + extra bytes
		LT_MEM_TRACK_ALLOC(m_pwfmt = (LWAVEFORMATEX *) LTMemAlloc (sizeof(LWAVEFORMATEX)+cbExtra),LT_MEM_TYPE_SOUND);
		if (m_pwfmt)
		{
			...
			// Read those extra bytes, append to LWAVEFORMATEX structure
			if (cbExtra != 0)
			{
				if ( mmioRead (m_hmmio, (LPSTR) ((BYTE *)(m_pwfmt) + sizeof (LWAVEFORMATEX)), cbExtra) != cbExtra)
				{
...
//	for( int32 i = 0; i < MAX_USER_DATA_INDEX + 1; m_userData[ i++ ] = 0 );
}
...
bool CSample::Init( HRESULT& hResult, LPDIRECTSOUND pDS, uint32 uiNumSamples,
				   bool b3DBuffer, LWAVEFORMATEX* pWaveFormat, LTSOUNDFILTERDATA* pFilterData )
{
	bool bUseFilter;
	...
	if( pWaveFormat == NULL )
	{
		...
		m_dsbDesc.lpwfxFormat = (LPWAVEFORMATEX)&m_waveFormat;
	}

	else
	{
		memcpy( &m_waveFormat, pWaveFormat, sizeof( LWAVEFORMATEX ) );
...
bool C3DSample::Init( HRESULT& hResult, LPDIRECTSOUND pDS, uint32 uiNumSamples, LWAVEFORMATEX* pWaveFormat, LTSOUNDFILTERDATA* pFilterData )
{
	Term( );
...
CSample* CDx8SoundSys::CreateBuffer( LWAVEFORMATEX* pWaveFormat, DWORD dwBufferSize, DWORD dwFlags )
{
	CSample* pSample;
...
void CDx8SoundSys::Reset( )
{
	...
	m_hResult = NULL;
	memset( &m_waveFormat, 0, sizeof( LWAVEFORMATEX ) );
	memset( &m_userPrefs, 0, ( MAX_USER_PREF_INDEX + 1 ) * sizeof( int32 ) );
	m_pStreams = NULL;
...
static bool QueryEAXSupport( IKsPropertySet& ksPropertySet, uint32 uiQuery )
{
	ULONG ulSupport = 0;
...
bool CDx8SoundSys::GetPropertySetForEAX( )
{
	...
	LWAVEFORMATEX wave;
	memset(&wave, 0, sizeof(LWAVEFORMATEX));
...
struct FormatEnumProcData
{
	...
	bool		m_bUseDriver;
	LWAVEFORMATEX m_pcmDestWaveFormat;
};
...
)
{
	...
	// realtime, skip it.
	MMRESULT mmResult = acmStreamOpen( NULL, pProcData->m_hDriver, pafd->pwfx, (LPWAVEFORMATEX) & pProcData->m_pcmDestWaveFormat, NULL, 0, 0, ACM_STREAMOPENF_QUERY);
	if( mmResult == 0 )
	{
...
static bool HasFormat( HACMDRIVERID hDriverId, WORD nFormatTag )
{
    ...
    if (dwSize < sizeof(LWAVEFORMATEX))
		dwSize = sizeof(LWAVEFORMATEX);
    LWAVEFORMATEX* pwf = ( LWAVEFORMATEX* ) new BYTE[dwSize];
    memset(pwf, 0, dwSize);
    pwf->cbSize = LOWORD(dwSize) - sizeof(LWAVEFORMATEX);
    pwf->wFormatTag = nFormatTag;
    ACMFORMATDETAILS fd;
    memset(&fd, 0, sizeof(fd));
    fd.cbStruct = sizeof(fd);
    fd.pwfx = (LPWAVEFORMATEX)pwf;
    fd.cbwfx = dwSize;
	...
	memset( &formatEnumProcData.m_pcmDestWaveFormat, 0, sizeof( LWAVEFORMATEX ) );
	formatEnumProcData.m_pcmDestWaveFormat.cbSize = sizeof( LWAVEFORMATEX );
...
int32	CDx8SoundSys::Startup( void )
{
...
uint32	CDx8SoundSys::MsCount( void )
{
//	Sleep( 20 );
	return ::timeGetTime( );
}

int32	CDx8SoundSys::SetPreference( uint32 uiNumber, int32 siValue )
{
...
int32	CDx8SoundSys::GetPreference( uint32 uiNumber )
{
	if( uiNumber > MAX_USER_PREF_INDEX )
		return 0;
...
void* CDx8SoundSys::MemAllocLock( uint32 uiSize )
{
	void* p;
...
int32	CDx8SoundSys::WaveOutOpen( LHDIGDRIVER* phDriver, PHWAVEOUT* pphWaveOut, int32 siDeviceId, LWAVEFORMAT* pWaveFormat )
{
...
#define CONVERT_LOG_VOL_TO_LIN_VOL( logVol, linVol )	\
	{	\
		...
		linVol = ( int32 )( ( MAX_MASTER_VOLUME * MIN_MASTER_VOLUME ) / ( MAX_MASTER_VOLUME - ( t * DEL_MASTER_VOLUME ) ) );	\
	}	\

void CDx8SoundSys::SetDigitalMasterVolume( LHDIGDRIVER hDig, int32 siMasterVolume )
{
	...
	if( siMasterVolume < ( int32 )MIN_MASTER_VOLUME )
		siMasterVolume = ( int32 )MIN_MASTER_VOLUME;

	if( siMasterVolume > ( int32 )MAX_MASTER_VOLUME )
		siMasterVolume = ( int32 )MAX_MASTER_VOLUME;
...
int32	CDx8SoundSys::GetDigitalMasterVolume( LHDIGDRIVER hDig )
{
	LPDIRECTSOUNDBUFFER pDSPrimaryBuffer = ( LPDIRECTSOUNDBUFFER )hDig;
...
int32	CDx8SoundSys::DigitalHandleRelease( LHDIGDRIVER hDriver )
{
	return 0;
}

int32	CDx8SoundSys::DigitalHandleReacquire( LHDIGDRIVER hDriver )
{
	return 0;
}
...
void CDx8SoundSys::Set3DProviderMinBuffers( uint32 uiMinBuffers )
{
	m_dwMinHardwareBuffers = uiMinBuffers;
}

// 3d sound provider functions
int32 CDx8SoundSys::Open3DProvider( LHPROVIDER hLib )
{
	// assume failure
...
int32	CDx8SoundSys::Enumerate3DProviders( LHPROENUM* phNext, LHPROVIDER* phDest, char** psName)
{
	int nCur = *phNext;
	phNext[0] += 1;
...
void CDx8SoundSys::Set3DUserData( LH3DPOBJECT hObj, uint32 uiIndex, int32 siValue )
{
...
int32	CDx8SoundSys::Get3DUserData( LH3DPOBJECT hObj, uint32 uiIndex )
{
	if( hObj == NULL || uiIndex > MAX_USER_DATA_INDEX )
		return 0;
...
LH3DSAMPLE CDx8SoundSys::Allocate3DSampleHandle( LHPROVIDER hLib )
{
	...
	memcpy( &p3DSample->m_sample.m_waveFormat, &m_waveFormat, sizeof( LWAVEFORMATEX ) );
...
int32 CDx8SoundSys::Init3DSampleFromAddress( LH3DSAMPLE hS, void* pStart, uint32 uiLen, LWAVEFORMATEX* pWaveFormat, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData )
{
//	LOG_WRITE( g_pLogFile, "SetSampleAddress( %x, %x, %d )\n", hS, pStart, uiLen );
	...
	// Modify the pitch.
	LWAVEFORMATEX waveFormat = *pWaveFormat;
	waveFormat.nSamplesPerSec = siPlaybackRate;
	waveFormat.nAvgBytesPerSec = waveFormat.nBlockAlign * waveFormat.nSamplesPerSec;

	if ( !p3DSample->Init( m_hResult, m_pDirectSound, uiLen, ( LWAVEFORMATEX* )&waveFormat, pFilterData ) )
		return LTFALSE;
...
int32	CDx8SoundSys::Init3DSampleFromFile( LH3DSAMPLE hS, void* pFile_image, int32 siBlock, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData  )
{
//	LOG_WRITE( g_pLogFile, "SetSampleFile( %x, %x, %d )\n", hS, pFile_image, siBlock );
	...
	LWAVEFORMATEX const* pWaveFormatEx = ( LWAVEFORMATEX* )pWaveFormat;
	...
	if( pWaveFormatEx->wFormatTag != WAVE_FORMAT_PCM )
	{
		...
		LWAVEFORMATEX pcmWaveFormat;
		memset( &pcmWaveFormat, 0, sizeof( LWAVEFORMATEX ) );

		pcmWaveFormat.cbSize = sizeof( LWAVEFORMATEX );
		...
		{
			// ADPCM compression
			mmResult = acmFormatSuggest( m_hAcmADPCMDriver, (LPWAVEFORMATEX)pWaveFormatEx,
				(LPWAVEFORMATEX)&pcmWaveFormat,	sizeof( LWAVEFORMATEX ), ACM_FORMATSUGGESTF_WFORMATTAG );
		...
		else if ( pWaveFormatEx->wFormatTag == WAVE_FORMAT_MPEGLAYER3 )
		{
			...
			mmResult = acmFormatSuggest( m_hAcmMP3Driver, (LPWAVEFORMATEX)pWaveFormatEx,
				(LPWAVEFORMATEX)&pcmWaveFormat,	sizeof( LWAVEFORMATEX ), ACM_FORMATSUGGESTF_WFORMATTAG );
...
*/
		...
		uint32 nFlags = 0;
		mmResult = acmStreamOpen( &hAcmStream, hDriver, (LPWAVEFORMATEX)( pWaveFormatEx ),
			(LPWAVEFORMATEX)&pcmWaveFormat, NULL, 0, 0, ACM_STREAMOPENF_QUERY );
		...
		mmResult = acmStreamOpen( &hAcmStream, hDriver, (LPWAVEFORMATEX)( pWaveFormatEx ),
			(LPWAVEFORMATEX)&pcmWaveFormat, NULL, 0, 0, nFlags );
...
//		}
	...
	{
		// Modify the pitch.
		LWAVEFORMATEX waveFormat = *pWaveFormatEx;
		waveFormat.nSamplesPerSec = siPlaybackRate;
		waveFormat.nAvgBytesPerSec = waveFormat.nBlockAlign * waveFormat.nSamplesPerSec;
...
int32	CDx8SoundSys::Get3DSampleVolume( LH3DSAMPLE hS )
{
...
void CDx8SoundSys::Set3DSampleVolume( LH3DSAMPLE hS, int32 siVolume )
{
	if( hS == NULL )
		return;
	...
	if( siVolume < ( int32 )MIN_MASTER_VOLUME )
		siVolume = ( int32 )MIN_MASTER_VOLUME;

	if( siVolume > ( int32 )MAX_MASTER_VOLUME )
		siVolume = ( int32 )MAX_MASTER_VOLUME;
...
void CDx8SoundSys::Set3DSampleMsPosition( LHSAMPLE hS, int32 siMilliseconds )
{
	if( hS == NULL )
		return;
...
int32	CDx8SoundSys::Set3DSampleInfo( LH3DSAMPLE hS, LTSOUNDINFO* pInfo )
{
    ...
    p3DSample->m_sample.m_waveFormat.nChannels = ( uint16 )pInfo->channels;
    p3DSample->m_sample.m_waveFormat.nSamplesPerSec = pInfo->rate;
    p3DSample->m_sample.m_waveFormat.wBitsPerSample = ( uint16 )pInfo->bits;
...
void CDx8SoundSys::Set3DSampleLoopBlock( LH3DSAMPLE hS, int32 siLoop_start_offset, int32 siLoop_end_offset, bool bEnable )
{
...
//	LOG_WRITE( g_pLogFile, "AllocateSampleHandle( %x )\n", hDig );
	...
	memcpy( &pSample->m_waveFormat, &m_waveFormat, sizeof( LWAVEFORMATEX ) );
...
void CDx8SoundSys::SetSampleVolume( LHSAMPLE hS, int32 siVolume )
{
...
int32	CDx8SoundSys::GetSampleVolume( LHSAMPLE hS )
{
//	LOG_WRITE( g_pLogFile, "GetSampleVolume( %x )\n", hS );
...
int32 ConvertLogPanToLinPan( long logPan )
{
	long linPan;
...
void CDx8SoundSys::SetSamplePan( LHSAMPLE hS, int32 siPan )
{
...
int32	CDx8SoundSys::GetSamplePan( LHSAMPLE hS )
{
//	LOG_WRITE( g_pLogFile, "GetSamplePan( %x )\n", hS );
	...
	int32 siPan = ConvertLogPanToLinPan( lDSPan );

	return siPan;
}

void CDx8SoundSys::SetSampleUserData( LHSAMPLE hS, uint32 uiIndex, int32 siValue )
{
...
int32 CDx8SoundSys::InitSampleFromAddress( LHSAMPLE hS, void* pStart, uint32 uiLen, LWAVEFORMATEX* pWaveFormat, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData )
{
//	LOG_WRITE( g_pLogFile, "InitSampleFromAddress( %x, %x, %d )\n", hS, pStart, uiLen );
	...
	waveFormat.nAvgBytesPerSec = waveFormat.nBlockAlign * waveFormat.nSamplesPerSec;
	CSample* pSample = ( CSample* )hS;
	if( !pSample->Init( m_hResult, m_pDirectSound, uiLen, false, ( LWAVEFORMATEX* )&waveFormat, pFilterData ))
		return LTFALSE;
...
int32	CDx8SoundSys::InitSampleFromFile( LHSAMPLE hS, void* pFile_image, int32 siBlock, int32 siPlaybackRate, LTSOUNDFILTERDATA* pFilterData )
{
...
*/
		...
		if ( pWaveFormatEx->wFormatTag == WAVE_FORMAT_IMA_ADPCM )
		{
			mmResult = acmStreamOpen( &hAcmStream, m_hAcmADPCMDriver, (LPWAVEFORMATEX)( pWaveFormatEx ),
				(LPWAVEFORMATEX)&pcmWaveFormat, NULL, 0, 0, 0 );
		}
		else if ( pWaveFormatEx->wFormatTag == WAVE_FORMAT_MPEGLAYER3)
		{
			mmResult = acmStreamOpen( &hAcmStream, m_hAcmMP3Driver, (LPWAVEFORMATEX)( pWaveFormatEx ),
				(LPWAVEFORMATEX)&pcmWaveFormat, NULL, 0, 0, 0 );
		}
...
void CDx8SoundSys::SetSampleLoopBlock( LHSAMPLE hS, int32 siLoop_start_offset, int32 siLoop_end_offset, bool bEnable )
{
	if( hS == NULL )
		return;
...
void CDx8SoundSys::SetSampleMsPosition( LHSAMPLE hS, int32 siMilliseconds )
{
...
int32	CDx8SoundSys::GetSampleUserData( LHSAMPLE hS, uint32 uiIndex )
{
...
{
	CSample* pSample = ( CSample* )hS;
	int32 siStatus = ( pSample->IsPlaying( ) ? LS_PLAYING : LS_STOPPED );
	return siStatus;
}
...
LHSTREAM CDx8SoundSys::OpenStream( char* sFilename, uint32 nFilePos, LHDIGDRIVER hDig, char* sStream, int32 siStream_mem )
{
...
//	streamBufferParams.m_siParams[SBP_BUFFER_SIZE] = STR_BUFFER_SIZE;
	...
	if ( pWaveFile->IsMP3() )
	{
		...
		mmResult = acmFormatSuggest( m_hAcmMP3Driver, (LPWAVEFORMATEX)pWaveFile->m_pwfmt, (LPWAVEFORMATEX)&pcmWaveFormat,
			sizeof( LWAVEFORMATEX ), ACM_FORMATSUGGESTF_WFORMATTAG );

		mmResult = acmStreamOpen( &hAcmStream, m_hAcmMP3Driver, (LPWAVEFORMATEX)pWaveFile->m_pwfmt, (LPWAVEFORMATEX)&pcmWaveFormat, NULL, 0, 0, 0 );
...
void CDx8SoundSys::SetStreamPlaybackRate( LHSTREAM hStream, int32 siRate )
{
	//OutputDebugString("OLD [CDx8SoundSys::SetStreamPlaybackRate]\n");
}

void CDx8SoundSys::SetStreamMsPosition( LHSTREAM hStream, int32 siMilliseconds )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);
...
void CDx8SoundSys::SetStreamUserData( LHSTREAM hS, uint32 uiIndex, int32 siValue)
{
	//OutputDebugString("OLD [CDx8SoundSys::SetStreamUserData]\n");
}

int32	CDx8SoundSys::GetStreamUserData( LHSTREAM hS, uint32 uiIndex)
{
...
void CDx8SoundSys::PauseStream( LHSTREAM hStream, int32 siOnOff )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);
...
void CDx8SoundSys::SetStreamVolume( LHSTREAM hStream, int32 siVolume )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);
...
void CDx8SoundSys::SetStreamPan( LHSTREAM hStream, int32 siPan )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);
...
int32 CDx8SoundSys::GetStreamVolume( LHSTREAM hStream )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);
...
int32 CDx8SoundSys::GetStreamPan( LHSTREAM hStream )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);
...
int32 CDx8SoundSys::GetStreamBufferParam( LHSTREAM hStream, uint32 uiParam )
{
	CWinSync_CSAuto cProtection(m_cCS_SoundThread);

	//OutputDebugString("NEW [CDx8SoundSys::GetStreamBufferParam]\n");
	CStream* pStream = ( CStream* )hStream;
	int32 siParamVal = ( uiParam < SBP_NUM_PARAMS ? pStream->m_streamBufferParams.m_siParams[ uiParam ] : 0 );
	return siParamVal;
}
...
int32	CDx8SoundSys::DecompressADPCM( LTSOUNDINFO* pInfo, void** ppOutData, uint32* puiOutSize )
{
	 return 0;
/*
	...
	LWAVEFORMATEX* pWaveFormatEx = ( LWAVEFORMATEX* )pWaveFormat;
...
//	mmResult = acmStreamOpen( &hAcmStream, m_hAcmADPCMDriver, pWaveFormatEx, &pcmWaveFormat, NULL, 0, 0, 0 );
	...
	uint32 uiWaveFmtSize = sizeof( LWAVEFORMATEX ) - sizeof( pcmWaveFormat.cbSize );
	uint32 uiFileSize =
...
int32	CDx8SoundSys::DecompressASI( void* pInData, uint32 uiInSize, char* sFilename_ext, void** ppWav, uint32* puiWavSize, LTLENGTHYCB fnCallback )
{
	bool bSuccess = false;
	...
	{
		mmResult = acmFormatSuggest( m_hAcmMP3Driver, (LPWAVEFORMATEX)( pWaveFormatEx ),
			(LPWAVEFORMATEX)&pcmWaveFormat,	sizeof( LWAVEFORMATEX ), nSuggestFlags );
